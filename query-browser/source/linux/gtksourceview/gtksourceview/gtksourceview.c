...
gtk_source_view_class_init (GtkSourceViewClass *klass)
{
	...
	g_object_class_install_property (object_class,
					 PROP_COMPOSE_MARKERS,
					 g_param_spec_boolean ("compose_line_markers",
							       _("Compose Line Markers"),
							       _("Whether to compose multiple line marker pixbufs"),
							       FALSE,
							       G_PARAM_READWRITE));
			      ...
			      GParamSpec   *pspec)
{
	...
	switch (prop_id)
	{
		...
		case PROP_COMPOSE_MARKERS:
			gtk_source_view_set_compose_line_markers (view,
                                                                  g_value_get_boolean (value));
			break;
			      ...
			      GParamSpec *pspec)
{
	...
	switch (prop_id)
	{
		...
		case PROP_COMPOSE_MARKERS:
			g_value_set_boolean (value,
					     gtk_source_view_get_compose_line_markers (view));
...
gtk_source_view_init (GtkSourceView *view)
{
	...
	view->priv->smart_home_end = TRUE;
        view->priv->compose_line_markers= TRUE;
	
	view->priv->pixmap_cache = g_hash_table_new_full (g_str_hash, g_str_equal,
							  (GDestroyNotify) g_free,
							  (GDestroyNotify) g_object_unref);

	view->priv->color_table = g_hash_table_new_full (g_str_hash, g_str_equal,
                                                         (GDestroyNotify) g_free,
                                                         (GDestroyNotify) g_free);
...
gtk_source_view_finalize (GObject *object)
{
	...
	if (view->priv->color_table)
		g_hash_table_destroy (view->priv->color_table);

        if (view->priv->highlight_gc)
                g_object_unref (G_OBJECT (view->priv->highlight_gc));
		   ...
		   gint           y)
{
	...
	do
	{
		...
		if (pixbuf)
		{
                        if (!view->priv->compose_line_markers)
                        {
                                gint prio= gtk_source_marker_get_priority (marker);
                                if (prio > current_prio)
                                {
                                        prio= current_prio;
                                        if (composite)
                                                g_object_unref (composite);
                                        composite= g_object_ref (pixbuf);
...
static GSList * 
draw_line_highlight (GtkSourceView *view,
                     GdkWindow     *window,
                     GSList        *current_marker,
                     gint          *line_number,
                     gint           y,
                     ...
                     gint           height)
{
	...
	const gchar *marker_type;
        ...
        GdkColor best_fg_color, best_bg_color;
        gint best_prio= -1;
	
	g_assert (current_marker);

        /* locate the color to use in this line */
	do
	{
		marker = current_marker->data;
		...
		marker_type = gtk_source_marker_get_marker_type (marker);
		if (gtk_source_view_get_marker_highlight_colors (view, marker_type, &fg_color, &bg_color))
                {
                        gint prio;
                        
                        prio= gtk_source_marker_get_priority (marker);
                        if (prio > best_prio)
                        {
                                best_prio= prio;
                                best_fg_color= fg_color;
	...
	while (current_marker);
        ...
        if (best_prio >= 0)
        {
                ...
                /* should be in realize, but since we don't have one, do it here */
                if (!view->priv->highlight_gc)
                  view->priv->highlight_gc= gdk_gc_new(window);
                gc= view->priv->highlight_gc;

                gdk_gc_set_foreground(gc, &best_bg_color);
                gdk_draw_rectangle(window, gc, TRUE, 0, y, width, height);
         
                gdk_gc_set_foreground(gc, &best_fg_color);
                gdk_draw_line(window, gc, 0, y, width, y);
                gdk_draw_line(window, gc, 0, y+height-1, width, y+height-1);
        }
...
static void
gtk_source_view_paint_highlight (GtkSourceView *source_view,
			         GdkEventExpose *event)
{
	GdkWindow *win;
        GtkTextView *view;
	GSList *markers, *current_marker;
        GtkTextIter first_line, last_line;
	gint marker_line = 0;
	gint y1, y2;
        gint width;
	gint i;
        
        view= GTK_TEXT_VIEW (source_view);

	win = gtk_text_view_get_window (view,
	...
	gtk_text_view_window_to_buffer_coords (view,
        ...
        /* get the iterators corresponding to the refreshed lines */
        gtk_text_view_get_iter_at_location (view, &first_line, 0, y1);
        gtk_text_view_get_iter_at_location (view, &last_line, 0, y2);
	

	/* get markers for the exposed region */
	markers = NULL;
        if (!gtk_text_iter_ends_line (&last_line))
                gtk_text_iter_forward_to_line_end (&last_line);

        markers = gtk_source_buffer_get_markers_in_region (
			source_view->priv->source_buffer, &first_line, &last_line);
	i = 0;
	current_marker = markers;
	if (current_marker)
		marker_line = gtk_source_marker_get_line (
	...
	while (current_marker)
	{
		gint y, height;
                ...
                gtk_source_buffer_get_iter_at_marker (source_view->priv->source_buffer,
                                                      &iter,
                                                      current_marker->data);
		...
		gtk_text_view_buffer_to_window_coords (view,
                ...
                /* draw markers for the line */
                current_marker = draw_line_highlight (source_view,
                                                      win,
                                                      current_marker,
                                                      &marker_line,
                                                      y,
			...
			GdkEventExpose *event)
{
	...
	else 
	{
		...
		if (event->window == gtk_text_view_get_window (text_view, GTK_TEXT_WINDOW_TEXT))
		{
			gtk_source_view_paint_highlight(view, event);
		}
...
gboolean
gtk_source_view_get_compose_line_markers (GtkSourceView *view)
{
	g_return_val_if_fail (view != NULL, FALSE);
	g_return_val_if_fail (GTK_IS_SOURCE_VIEW (view), FALSE);

	return view->priv->compose_line_markers;
}

void
gtk_source_view_set_compose_line_markers (GtkSourceView *view,
                                          gboolean       compose)
{
	g_return_if_fail (view != NULL);
	g_return_if_fail (GTK_IS_SOURCE_VIEW (view));
	...
	compose = (compose != FALSE);
        ...
        if (compose != view->priv->compose_line_markers)
        {
                view->priv->compose_line_markers = compose;

                /* force expose event, which will adjust margin. */
                gtk_widget_queue_draw (GTK_WIDGET (view));

                g_object_notify (G_OBJECT (view), "compose_line_markers");
	}
}
...
void             
gtk_source_view_set_marker_highlight_colors	(GtkSourceView   *view,
							 const gchar     *marker_type,
							 GdkColor        *fg_color,
                                                         GdkColor	 *bg_color)
{
        GtkSourceViewHighlightColors *colors;

	
	if (fg_color && bg_color)
	{
                colors = g_new0(GtkSourceViewHighlightColors, 1);
                colors->fore_color= *fg_color;
                colors->back_color= *bg_color;

		g_hash_table_insert (view->priv->color_table,
				     g_strdup (marker_type),
				     colors);
	}
	else
	{
		g_hash_table_remove (view->priv->color_table, marker_type);
	}
}

gboolean	
gtk_source_view_get_marker_highlight_colors	(GtkSourceView   *view,
				       			 const gchar     *marker_type,
							 GdkColor	 *fg_color,
							 GdkColor	 *bg_color)
{
	...
	g_return_val_if_fail (view != NULL, FALSE);
	g_return_val_if_fail (GTK_IS_SOURCE_VIEW (view), FALSE);
	g_return_val_if_fail (marker_type != NULL, FALSE);
	
	colors = g_hash_table_lookup (view->priv->color_table, marker_type);
        if (colors)
        {
                if (fg_color)
                        *fg_color= colors->fore_color;
                if (bg_color)
                        *bg_color= colors->back_color;
                return TRUE;
        }
