...
type
 ...
 TStringFunction = function : string; // added by MikeZ
...
{$ENDIF}
 ...
 TRegExpr = class
   ...
   public
    ...
    function ReplaceWithCallback (AInputStr : RegExprString;                  // added by MikeZ
      CallbackFunc: TStringFunction;                                          // added by MikeZ
      AUseSubstitution : boolean{$IFDEF D4_}= False{$ENDIF}) : RegExprString; // added by MikeZ
...
// added by MikeZ - start

function TRegExpr.ReplaceWithCallback (AInputStr : RegExprString;
      CallbackFunc: TStringFunction;
  ...
  if Exec (AInputStr) then
   REPEAT
    Result := Result + System.Copy (AInputStr, PrevPos,
      MatchPos [0] - PrevPos);
    if AUseSubstitution //###0.946
    then Result := Result + Substitute (CallbackFunc)
    else Result := Result + CallbackFunc;
    PrevPos := MatchPos [0] + MatchLen [0];
   UNTIL not ExecNext;
  Result := Result + System.Copy (AInputStr, PrevPos, MaxInt); // Tail
 end; { of function TRegExpr.Replace
