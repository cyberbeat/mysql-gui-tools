...
interface
uses
  SysUtils, Classes, Controls, SciLexer, SciLexerMemo,SciLexerMod,DB,DBCtrls,Messages;

type
  TScintillaDB = class(TScintilla)
  private
...
constructor TScintillaDB.Create(AOwner: TComponent);
begin
...
procedure TScintillaDB.KeyUp(var Key: Word; Shift: TShiftState);
begin
  if Key=27 then
  begin
    Inherited;
...
destructor TScintillaDB.Destroy;
begin
  FDataLink.Free;
  FDataLink := nil;
  inherited Destroy;
...
procedure TScintillaDB.CMEnter(var Msg: TCMEnter);
begin
  SetFocused(True);
  inherited;
end;

procedure TScintillaDB.CMExit(var Msg: TCMExit);
begin
  try
...
procedure TScintillaDB.CMGetDataLink(var Msg: TMessage);
begin
  Msg.Result := Integer(FDataLink);
end;

procedure TScintillaDB.DataChange(Sender: TObject);
begin
...
procedure TScintillaDB.EditingChange(Sender: TObject);
begin
...
function TScintillaDB.GetDataField: string;
begin
  Result := FDataLink.FieldName;
end;

function TScintillaDB.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

function TScintillaDB.GetField: TField;
begin
  Result := FDataLink.Field;
end;

function TScintillaDB.GetReadOnly: Boolean;
begin
  Result := FDataLink.ReadOnly;
end;

procedure TScintillaDB.doSciModified(const position : LongInt; const modificationType : LongInt;
														text : PChar; const length : LongInt; const linesAdded : LongInt; const line : LongInt;
...
procedure TScintillaDB.LoadMemo;
var
  BlobStream: TStream;
begin
  try
...
procedure TScintillaDB.Notification(AComponent: TComponent;
...
procedure TScintillaDB.SetDataField(const Value: string);
begin
  FDataLink.FieldName := Value;
end;

procedure TScintillaDB.SetDataSource(Value: TDataSource);
begin
...
procedure TScintillaDB.SetFocused(Value: Boolean);
begin
...
procedure TScintillaDB.SetReadOnly(Value: Boolean);
begin
  FDataLink.ReadOnly := Value;
end;

procedure TScintillaDB.UpdateData(Sender: TObject);
var
  BlobStream: TStream;
begin
  if FDataLink.Field.IsBlob then
