...
namespace tut
{
  ...
  /**
   * Exception to be thrown when test desctructor threw an exception.
   */
  ...
  /**
   * Exception to be thrown when test threw SEH (Win32)
   */
  ...
  struct group_base
  {
    ...
    static int get_disabled_groups_count() { return disabled_groups_count_; }

    bool is_disabled() const { return disabled_; }

  protected:

    void inc_disabled_groups_count() { ++disabled_groups_count_; }
    void set_disabled(bool d) { disabled_= d; }

  private:

    bool disabled_;
    static int disabled_groups_count_;
  };
  ...
  class test_runner
  {
    ...
    void register_group(const std::string& name,group_base* gr)
    {
      if( gr == 0 )
      {
        throw std::invalid_argument("module shall be non-null");
      }

      groups::iterator found = groups_.find(name);
      if( found != groups_.end() )
      {
        std::string msg("attempt to add already existent module "+name);
  ...
  namespace 
  {
    ...
    {
      ensure_equals<>(NULL, actual, expected);
    }

    // Helper method for string replacement.
    std::string replace_string(const std::string text, const std::string &search_text, const std::string &replace_string)
    {
      std::string result(text);
      std::string::size_type pos = result.find(search_text, 0);
      int length = search_text.length();

      while (std::string::npos != pos )
      {
        result.replace(pos, length, replace_string);
        pos = result.find(search_text, 0);
      }

      return result;
    }

    void ensure_equals_ignore_line_breaks(const char* msg, const std::string& actual, const std::string& expected)
    {
      std::string actual_mod= replace_string(actual, "\r\n", "\n");
      std::string expected_mod= replace_string(expected, "\r\n", "\n");
      ensure_equals<>(msg, actual_mod, expected_mod);
    }
   ...
   * Data base class.
   */
  template <class Data,int MaxTestsInGroup = MAX_TESTS_IN_GROUP>
