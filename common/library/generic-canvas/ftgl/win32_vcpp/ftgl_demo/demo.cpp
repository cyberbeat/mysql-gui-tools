...
const bool UseFontDisplayLists = true;
static GLint SceneList = 0;
...
// YOU'LL PROBABLY WANT TO CHANGE THESE
	...
	#define DEFAULT_FONT "/Users/henry/Development/PROJECTS/FTGL/ftglcvs/FTGL/demo/Tahoma.ttf"

#endif

#ifdef WIN32

	#define DEFAULT_FONT "C:\\Windows\\Fonts\\Trebucbd.ttf"
	using namespace std;
#endif
...

{
    ...
    fonts[0]->UseDisplayList(UseFontDisplayLists);
    fonts[1] = new FTGLPolygonFont( font_filename );
    fonts[1]->UseDisplayList(UseFontDisplayLists);
    fonts[2] = new FTGLTextureFont( font_filename );
    fonts[2]->UseDisplayList(UseFontDisplayLists);
    fonts[3] = new FTGLBitmapFont( font_filename );
    fonts[3]->UseDisplayList(UseFontDisplayLists);
    fonts[4] = new FTGLPixmapFont( font_filename );
    fonts[4]->UseDisplayList(UseFontDisplayLists);
...
void draw_scene()
{
  ...
  //strncpy(string[0], "Ein langer String zum testen.\0", 30);
   strncpy(string[1], "ABCDEFGHIJKLMNOP\0", 17);
