...
const FTPoint& FTTextureGlyph::Render( const FTPoint& pen)
{
    ...
    glTranslated( pen.X(),  pen.Y(), 0.0f);

    glBegin( GL_QUADS);
        glTexCoord2d( uv[0].X(), uv[0].Y());
        glVertex2d( pos.X(), pos.Y());

        glTexCoord2d( uv[0].X(), uv[1].Y());
        glVertex2d( pos.X(), pos.Y() - destHeight);

        glTexCoord2d( uv[1].X(), uv[1].Y());
        glVertex2d( destWidth + pos.X(), pos.Y() - destHeight);
        
        glTexCoord2d( uv[1].X(), uv[0].Y());
        glVertex2d( destWidth + pos.X(), pos.Y());
    glEnd();
