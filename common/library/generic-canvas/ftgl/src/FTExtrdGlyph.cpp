    ...
    for( tesselationIndex = 0; tesselationIndex < mesh->TesselationCount(); ++tesselationIndex)
    {
        ...
        glBegin( polyonType);
            for( unsigned int pointIndex = 0; pointIndex < subMesh->PointCount(); ++pointIndex)
            {
                ...
                glTexCoord2d( point.X() / horizontalTextureScale,
                              point.Y() / verticalTextureScale);
                
                glVertex3d( point.X() / 64.0f,
                            point.Y() / 64.0f,
                            0.0f);
            }
        glEnd();
    ...
    for( tesselationIndex = 0; tesselationIndex < mesh->TesselationCount(); ++tesselationIndex)
    {
        ...
        glBegin( polyonType);
            for( unsigned int pointIndex = 0; pointIndex < subMesh->PointCount(); ++pointIndex)
            {
                ...
                glTexCoord2d( subMesh->Point(pointIndex).X() / horizontalTextureScale,
                              subMesh->Point(pointIndex).Y() / verticalTextureScale);
                
                glVertex3d( subMesh->Point( pointIndex).X() / 64.0f,
        ...
        glBegin( GL_QUAD_STRIP);
            for( unsigned int j = 0; j <= numberOfPoints; ++j)
            {
                ...
                if( contourFlag & ft_outline_reverse_fill)
                {
                    ...
                    glVertex3d( point.X() / 64.0f, point.Y() / 64.0f, 0.0f);
                    glVertex3d( point.X() / 64.0f, point.Y() / 64.0f, -depth);
                }
                else
                {
                    glTexCoord2d( point.X() / horizontalTextureScale,
...
{
    double vectorX = a.X() - b.X();
    double vectorY = a.Y() - b.Y();
