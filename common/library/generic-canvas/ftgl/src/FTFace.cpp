    ...
    err(0)
{
    ...
    err = FT_New_Face( *FTLibrary::Instance().GetLibrary(), fontFilePath, 0, ftFace);
    if( err)
    {
        delete ftFace;
        ftFace = 0;
    ...
    err(0)
{
  ...
  err = FT_New_Face_From_FOND( *FTLibrary::Instance().GetLibrary(), fond, -1, ftFace);
  if( err)
  {
    printf("error %i\n", err);
    delete ftFace;
...
bool FTFace::Attach( const unsigned char *pBufferBytes, unsigned int bufferSizeInBytes)
{
    FT_Open_Args open;
