...
static MYX_GRT_SHELL_COMMAND_HELP_TEXT help_shell[] =
{
  ...
  },
  {
    ...
    "objectName     The name of the object to change into relative from the"NL
      ...
      "path           Absolut (starting with /) or relative path (starting with ./)"NL
...
{ 
  ...
  {
    "getListRefValueByObjName",
    "Returns a reference list value object based on its name.",
    "grtV.getListRefValueByObjName (grtList, \"name\")",
    "grtList        The GRT list to check."NL
      "name           The object name to search",
    NULL,
    NULL
  ...
  {
    "getGlobal",
    "Returns the global GRT object at the given path. Global objects are refered by a tree that can be accessed from all languages bound to the GRT.",
    "grtV.getGlobal (path)",
  ...
  {
    "lookupAdd",
    "Adds the given object to the global object id cache.",
    "grtV.lookupAdd (obj)",
    "obj             The object to add to the cache.",
    "  grtV.lookupAdd(conn)"NL
    "               Adds the object conn to the cache.",
    NULL
  },  
  {
    "lookupId",
    "Looks up the id in the global object id cache and returns the value if it's found.",
    "grtV.lookupId (id)",
    "id              The id of the object lookup.",
    "  print(grtV.lookupId(obj.refId))"NL
    "               Prints the object referenced by refId.",
    NULL
  },  
  {
    "newDict",
  ...
  {
    "newObj",
    "Creates a new GRT dict that is filled with the given values. All simple values (\"int\", \"string\", \"real\") are initialized and all lists are created.",
    "grtV.newObj (structName, name, _id, ownerId)",
    "structName         Struct name of the object to create"NL
      "name               Name of the object."NL
      "_id                The _id of the object. If \"\" is passed a new GUID is created"NL
      "ownerId            The _id of the owner of \"\"",
    "  rdbmsMgmt= grtV.newObj(\"db.mgmt.Management\", "NL
      "    \"rdbmsManagement\", grt.newGuid(), \"\")"NL
      "                   Create a new object from the struct \"db.mgmt.Management\"",
    NULL
  },
  {
    "remove",
    "Removes the list item with the giveb index.",
    "grtV.remove (index)",
  ...
  {
    "setContentType",
    "Sets the content type of an existing list or dict.",
    "grtV.setContentType(obj, contentType [, contentStructName])",
    "obj                The list or dict."NL
      "contentType        The content type that should be assigned to the object."NL
      "contentStructName  If the content type is \"dict\" a content struct name"NL
      "                   can be set. Only dicts with this struct assigned can"NL
      "                   be added to the list then.",
    NULL,
    NULL
  },
  {
  ...
  },
  {
    ...
    "typeName       the name of the type: int, real, string, list, dict",
    NULL,
    NULL
  },  
  {
...
{ 
  ...
  {
    "getCaption",
    "Returns the struct's cpation as a Lua string.",
    "grtS.getCaption (structName)",
    NULL,
    NULL,
    NULL
  },
  ...
  {
    "inheritsFrom",
    "Returns true if the given struct inherits from the given parent struct.",
    "grtS.inheritsFrom (structName, parentStructName)",
    NULL,
    NULL,
    NULL
  },
  ...
  {
    "set",
    "Sets the object's struct name.",
    "grtS.set (obj, structName)",
    NULL,
    NULL,
    NULL
  },
  {
    "validate",
    "Validates the given object against its struct. If an explicit struct name is given the object is validated against that struct.",
    "grtS.validate (obj [, structName])",
    NULL,
    NULL,
    NULL
  },
...
{ 
  {
    ...
    "grtM.callFunction (moduleName, functionName, arguments)",
    NULL,
    NULL,
    NULL
  },
...
void myx_grt_shell_show_command_help_print(MYX_GRT *grt, const char *group_name, const char *cmd)
{
  ...
  {
    if (strcmp2(group_name, help_group.group_name) == 0)
    {
      ...
      if (!cmd)
      {
        ...
        {
          dir= str_g_append_and_free(dir, str_align_left(help_text.cmd, 24, ' '));
          if ((i + 1) % 3 == 0)
            dir= str_g_append(dir, NL);
        }

        myx_grt_printf(grt, dir);

        myx_grt_printf(grt, NLNL
      ...
      else
      {
        for(i= 0; (help_text= help_group.commands[i]).cmd; i++)
        {
          if (strcmp2(help_text.cmd, cmd) == 0)
          {
            char ul[80];
            unsigned int k;
            ...
            myx_grt_printf(grt, NL"%s - %s.%s"NL, help_group.group_caption, help_group.group_name, help_text.cmd);
            myx_grt_printf(grt, ul);
            myx_grt_printf(grt, NL);
            ...
            if (help_text.desc)
            {
              ...
              myx_grt_printf(grt, "%s" NLNL, desc);

              g_free(desc);
            }

            myx_grt_printf(grt, "%s" NL, help_text.syntax);

            if (help_text.examples)
              myx_grt_printf(grt, NL "Examples:" NL "%s" NL, help_text.examples);

            if (help_text.see_also)
              myx_grt_printf(grt, NL "See also:" NL "%s" NL, help_text.see_also);
  ...
  if (!found)
    myx_grt_printf(grt, "Unknown command or function." NL);
}

void myx_grt_shell_show_command_help(MYX_GRT *grt, const char *command)
{
  if ((strcmp2(command, "quit") == 0) || (strcmp2(command, "exit") == 0) ||
    (strcmp2(command, "ls") == 0) || (strcmp2(command, "cd") == 0) ||
    (strcmp2(command, "show") == 0) || (strcmp2(command, "run") == 0) ||
    (strcmp2(command, "/path") == 0))
  {
