...
INSERT INTO `sakila`.`film` VALUES  (604,'MULAN MOON','A Emotional Saga of a Womanizer And a Pioneer who must Overcome a Dentist in A Baloon',2006,1,NULL,4,'0.99',160,'10.99','G','Behind the Scenes','2006-02-15 05:03:42'),
...
INSERT INTO `sakila`.`film` VALUES  (904,'TRAIN BUNCH','A Thrilling Character Study of a Robot And a Squirrel who must Face a Dog in Ancient India',2006,1,NULL,3,'4.99',71,'26.99','R','Trailers,Deleted Scenes','2006-02-15 05:03:42'),
...
--
-- Definition of trigger `sakila`.`upd_film`
--

DROP TRIGGER /*!50030 IF EXISTS */ `sakila`.`upd_film`;
...
--
-- Definition of trigger `sakila`.`del_film`
--

DROP TRIGGER /*!50030 IF EXISTS */ `sakila`.`del_film`;
...
--
-- Definition of function `sakila`.`get_customer_balance`
--
...
CREATE DEFINER=`root`@`%` FUNCTION  `sakila`.`get_customer_balance`(p_customer_id INT, p_effective_date DATETIME) RETURNS decimal(5,2)
    READS SQL DATA
    DETERMINISTIC
BEGIN
...
--
-- Definition of procedure `sakila`.`film_not_in_stock`
--
...
/*!50003 SET @TEMP_SQL_MODE=@@SQL_MODE, SQL_MODE='STRICT_TRANS_TABLES,STRICT_ALL_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,TRADITIONAL,NO_AUTO_CREATE_USER' */ $$
CREATE DEFINER=`root`@`%` PROCEDURE  `sakila`.`film_not_in_stock`(IN p_film_id INT, IN p_store_id INT, OUT p_film_count INT)
    READS SQL DATA
BEGIN
     SELECT inventory_id
     FROM inventory
